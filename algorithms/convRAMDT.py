...
class lstmramdt:
    ...
    def train(self, save, filename, split):
        ...
        self.model.add(Conv1D(filters=32, kernel_size=5, activation='relu', input_shape=(X_train.shape[-2:])))
        #self.model.add(Dropout(0.2))
    ...
    def data_preparation(self, db, train=False):
        ...
        if train:
            ...
            data_1 = np.array([[0,0],[10,10]])
            
            self.mmscaler_ram.fit(np.array(data)[:, np.newaxis])
            

            self.mmscaler.fit(data_1)
